...
if st.session_state.is_logged_in:
    with tab_dashboard:
        ...
        with col1:
            st.metric("📦 طلبات المنتجات", len(st.session_state.product_orders))

        with col2:
            total_product_revenue = sum(order['total_price'] for order in st.session_state.product_orders)
            st.metric("💰 إيرادات المنتجات", f"{total_product_revenue:,} ج.م")
        ...
        with col4:
            ...
            st.metric("💰 إيرادات الباقات", f"{total_sub_revenue:,} ج.م")
    ...
    with tab_about:
        ...
        with col3:
            ...
            <div style='text-align: center; padding: 30px; background: white;
                ...
                <div style='font-size: 4rem;'>🚗</div>
                <h3>توصيل سريع</h3>
                <p>خدمة توصيل سريعة لجميع أنحاء الإسكندرية برسوم مناسبة</p>
            </div>
            """, unsafe_allow_html=True)
...
if st.session_state.show_adoption_form:
    ...
    with st.form("adoption_form"):
        ...
        with col1:
            pet_type = st.selectbox("نوع الحيوان *:", ["قطة 🐱", "كلب 🐕", "طائر 🐦", "أرنب 🐰", "سلحفاة 🐢", "أخرى"])

        with col2:
            pet_age = st.selectbox("العمر المفضل:", ["صغير (أقل من سنة)", "متوسط (1-3 سنوات)", "كبير (أكثر من 3 سنوات)", "لا يهم"])

        adopt_experience = st.radio("هل لديك خبرة سابقة في تربية الحيوانات؟", ["نعم، لدي خبرة", "لا، هذه أول مرة", "لدي خبرة بسيطة"])
