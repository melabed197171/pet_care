...
# إعداد الصفحة
st.set_page_config(page_title="مركز الرعاية المتكاملة", layout="wide", page_icon="🐾")
...
def save_subscription(package_name, package_price, customer_info):
    try:
        ...
        notification = {
            "message": f"اشتراك جديد في باقة {package_name}",
...
with col_text:
    st.markdown("<h1>مركز الرعاية الصحية الشاملة 🐾</h1>", unsafe_allow_html=True)
...
st.write("### خيارات التواصل والعلاقات العامة")
...
with tab1:
    ...
    for idx, (package_name, package_data) in enumerate(st.session_state.packages.items()):
        ...
        st.write(f"## باقة {package_name} {package_data['icon']}")
        ...
        if st.button(f"تفعيل باقة {package_name} ✅", key=f"btn_{package_name}_{idx}"):
            if customer_name.strip() and customer_phone.strip():
                ...
                if save_subscription(package_name, package_data['price'], customer_info):
                    st.success(f"تم تفعيل باقة {package_name} بنجاح ✅")
                    st.balloons()
                    st.info(f"تم حفظ بيانات: {customer_name} - {customer_phone} 🔔")
                    st.rerun()
                else:
