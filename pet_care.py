...
# إعداد الصفحة
st.set_page_config(page_title="مركز العناية المتكاملة البيطرية", page_icon="🐾", layout="centered")
try:
    # عرض الصورة
    image = Image.open('cat.png')
    st.image(image, width=250)
        # إنشاء أعمدة للأزرار لتظهر بجانب بعضها
    col1, col2 = st.columns(2)
    with col1:
        if st.button("❤️ أعجبني"):
            st.balloons() # تأثير بالونات احتفالي
            st.success("شكراً على حبك للأليف!")
    with col2:
        if st.button("🏠 تبنَّ الآن"):
            st.info("سيتم التواصل معك لإتمام إجراءات التبني قريباً.")
except FileNotFoundError:
    st.warning("لم يتم العثور على ملف cat.png")
...
st.title("🐾مركز الرعاية المتكاملة البيطرية ")
st.markdown("""
    **تحت اشراف خبرة من:**
...
st.title("🐾 مركز الرعاية المتكاملة البيطرية")
st.subheader("تحت اشراف نخبة من أطباء والمهندسين")
...
---
📞 **للدعم الفني:** 01022395878xxxxxxxxx
""")